import numpy as np
import scipy.stats
from metrics import split_dataset
...
class TreeBuilderCART(object):
    ...
    def find_best_split(self, X, y, X_idx_sorted, mask):
        ...
        for feature_id in self.get_feature_ids(n_features):
            ...
            #gain = s_left * n_left + s_right * n_right
            #gain = s2_left / n_left + s2_right / n_right - (s_left**2) / (n_left**2) + (s_right**2) / (n_right**2)

            ind = np.argmax(gain[self.min_samples_per_leaf:-self.min_samples_per_leaf]) + self.min_samples_per_leaf
            #ind = min(int(x_mf.shape[0] / 2), x_mf.shape[0] - 2)
            ...
            #p_left = (float(ind) / y_mf.shape[0])
            #p_right = 1.0 - p_left
            #gain = np.var(y_mf) - p_left * np.var(y_mf[:ind + 1]) - p_right * np.var(y_mf[ind+1:])
