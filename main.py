...
def main(pathToTrain, pathToTest):
    ...
    clf = blending.Blending(
        LogisticRegression(), 
        [
            boosting.GradientBoosting(T=100), 
    ...
    sklearn_clf = blending.Blending(
        LogisticRegression(), 
        [
            GradientBoostingClassifier(criterion='mse', n_estimators=100, presort=True, subsample=1), 
            MLPClassifier(hidden_layer_sizes=(10,))
        ]
    )
    '''
    ...
    plt.show()
    #report(y_test, sklearn_y_pred, "sklearn")
    #report(y_test, y_pred, "self")
    return 0
